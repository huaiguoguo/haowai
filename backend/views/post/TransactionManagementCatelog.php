...
<div class="nav">
	<ul class="cc">
		<li><a href="<?php echo HostName_Backed;?>/default/TransactionManagement">事务办理文章</a></li>
		<li class="current"><a href="<?php echo HostName_Backed;?>/default/TransactionManagementCatelog">事务办理分类</a></li>
	</ul>
</div>
<form class="J_ajaxForm" data-role="list" action="<?php echo HostName_Backed;?>/default/sbmtzixuncatelog" method="post">
	<div class="mb10">
	...
	<div class="table_list">
		...
		<thead>
			<tr>
				<td></td>
				<td><span class="mr5"></span>版块名称</td>
				<td class="tar">fid</td>
				<td>文章数</td>
				<td>操作</td>
			</tr>
		...
		<?php foreach($catedb as $key => $cate):?>
			<?php $cateClass = isset($catelist[$cate['id']]) ? ' away_icon' : ' zero_icon';?>
		<tbody>
			<tr id="J_forum_tr_<?php echo $cate['id'];?>">
				...
				<span class="mr10 J_forum_names" data-id="<?php echo $cate['id'];?>"><?php echo $cate['catalog_name'];?></span>
				<a style="display:none" data-id="<?php echo $cate['id'];?>" data-type="forum_child" data-forumlevel="2" data-html="tbody" href="#" class="link_add J_addChild">添加子分类</a>
				</td>
				<td class="tar"><?php echo $cate['id'];?></td>
				<td></td>
				<td>
				<a href="javascript:void(0)" target="_blank" class="mr5">[访问]</a>
				<a href="<?php echo HostName_Backed;?>/default/deleteforum?fid=<?php echo $cate['id'];?>" class="mr5 J_ajax_del" data-pdata="{'fid': '<?php echo $cate['id'];?>'}">[删除]</a>
				</td>
			</tr>
		</tbody>
		<?php if(isset($catelist[$cate['id']]) && $catelist[$cate['id']]):?>
		<tbody id="J_table_list_<?php echo $cate['id'];?>">
			<?php //var_dump($catelist);exit;?>
			<?php foreach($catelist[$cate['id']] as $ke=>$form):?>
			<tr id="J_forum_tr_<?php echo $form['id'];?>">
				<td></td>
				<td>
				<?php echo $lang[$form['type']];?><span class="plus_icon plus_end_icon"></span>
				<span class="mr10 J_forum_names" data-id="<?php echo $form['id'];?>"><?php echo $form['catalog_name'];?></span>
				...
				<td>
				<a href="javascript:void(0)" target="_blank" class="mr5">[访问]</a>
				<a href="<?php echo HostName_Backed;?>/default/deleteforum?fid=<?php echo $form['id'];?>" class="mr5 J_ajax_del" data-pdata="{'fid': '<?php echo $form['id'];?>'}">[删除]</a>
				</td>
			</tr>
			<?php endforeach;?>
...
$(function(){
	...
	$('#J_table_list').on('blur', '.J_forum_names_input', function() {
		...
		//判断版块名是否修改过
		if($this.val() !== org_val) {
			$.post("<?php echo HostName_Backed;?>/default/Editname", {fid: $this.data('id'), name: $this.val() }, function(data){
