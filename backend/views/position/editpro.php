...
<div class="row-fluid">
  <div class="span12">
    <div class="box">
      <div class="box-title">
        <div class="span10">
          <h3><i class="icon-cog"></i>编辑产品</h3>
        </div>
        <div class="span2">
          <a class="btn" href="Javascript:window.history.go(-1)">返回</a>
        </div>
      </div>
      <form action="<?=HostName_Backed;?>/product/editproduct" method="post" class="form-horizontal form-validate">
        <input type="hidden" name="dosubmit" value="1">
        <div class="box-content">
          <div class="control-group">
            <label for="classname" class="control-label">产品名称：</label>
            <div class="controls">
              <input type="text" id="pname" name="pname" value="<?=$SiteInfo->pname;?>" class="input-medium" data-rule-required="true" />
          ...
          <div class="control-group">
            ...
            <div class="controls">
              <input type="text" id="price" name="price" value="<?=$SiteInfo->price;?>" class="input-medium" data-rule-required="true" />
              <span class="maroon">*</span><span class="help-inline"></span>
            </div>
          </div>
          <div class="control-group">
            ...
            <div class="controls">
              <img type="img" src="<?php if(!$SiteInfo->image):?><?=AdminAsset;?>/admin/img/home-300200.jpg<?php else:?><?=$SiteInfo->image;?><?php endif;?>" style="max-height:50px;" />
              <input type="hidden" id="picurl" name="picurl" value="<?php if(!$SiteInfo->image):?><?=AdminAsset;?>/admin/img/home-300200.jpg<?php else:?><?=$SiteInfo->image;?><?php endif;?>" class="input-medium" />
          ...
          <div class="control-group">
            ...
            <div class="controls">
              <img type="img" src="<?php if(!$SiteInfo->image):?><?=AdminAsset;?>/admin/img/home-300200.jpg<?php else:?><?=$SiteInfo->image;?><?php endif;?>" style="max-height:50px;" />
              <input type="hidden" id="jietu_url" name="jietu_url" value="<?php if(!$SiteInfo->image):?><?=AdminAsset;?>/admin/img/home-300200.jpg<?php else:?><?=$SiteInfo->image;?><?php endif;?>" class="input-medium" />
              <a class="btn insertimage">选择封面</a>
              <span class="help-inline">建议尺寸：宽300像素，高200像素</span>
            </div>
          </div>
          ...
          <div class="control-group">
            <label for="classname" class="control-label">应用介绍:</label>
              <div class="controls">
                <textarea id="info" name="a_instruction" data-rule-required="true"><?=$SiteInfo->desc;?></textarea>
              </div>
          </div>
...
<script type="text/javascript">
  ...
  KISSY.use('gallery/pageNotification/1.0/index', function (S, PageNotification) {
   ...
   $(".sub").on('click',function(){
    ...
    var method = 'POST';
    var url         = '<?=HostName_Backed;?>/product/editproduct?dosubmit=1';
    var param   = "dosubmit=1&pname="+pname+"&category_id="+category_id+"&id="+<?=$SiteInfo->id;?>+"&desc="+desc+"&picurl="+picurl+"&jietu_url="+jietu_url;
    var param   = {dosubmit:1,pname:pname,category_id:category_id,id:<?=$SiteInfo->id;?>,desc:desc,picurl:picurl,price:price};
    var result    = '';
    result = diyAjax(method,url,param);
    if(result==1){
      G.ui.tips.suc('编辑产品成功', '/backed/product/productlist');
      //pageNotification.success("添加分类成功", "提示");
    }else{
      G.ui.tips.die('编辑产品失败', '/backed/product/productlist');
