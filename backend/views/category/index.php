<?php
$this->renderpartial('/include/header');?><!--加载头部-->
<div class="nav">
    <ul class="cc">
        <li class="current"><a href="<?php echo HostName_Backed;?>/Category/index">分类管理</a></li>
    </ul>
</div>
<form class="J_ajaxForm" data-role="list" action="<?php echo HostName_Backed;?>/Category/sbmtkuaixuncatelog" method="post">
    <div class="mb10">
    ...
    <div class="table_list">
        ...
        <thead>
            <tr>
                <td></td>
                <td><span class="mr5"></span>分类名称</td>
                <td class="tar">fid</td>
                <td>文章数</td>
                <td>操作</td>
            </tr>
        ...
        <?php foreach($categorylist as $key => $cate):?>
            <?php $cateClass = isset($childrenlist[$cate['id']]) ? ' away_icon' : ' zero_icon';?>
        <tbody>
            <tr id="J_forum_tr_<?php echo $cate['id'];?>">
                <td><span data-id="<?php echo $cate['id'];?>" class="J_start_icon <?php echo $cateClass;?>" title="双击进行修改"></span></td>
                <td>
                <!-- <input type="text" class="input mr5" name="vieworder[1]" value="0" style="width:20px;"> -->
                <span class="mr10 J_forum_names" data-id="<?php echo $cate['id'];?>"><?php echo $cate['category_name'];?></span>
                <a style="display:none" data-id="<?php echo $cate['id'];?>" data-type="forum_child" data-forumlevel="1" data-html="tbody" href="#" class="link_add J_addChild">添加子分类</a>
                </td>
                <td class="tar"><?php echo $cate['id'];?></td>
                <td></td>
                <td><a href="javascript:void(0)" target="_blank" class="mr5">[访问]</a><a href="<?php echo HostName_Backed;?>/CommunityManagement/DeleteKuaixunCategory?sid=<?php echo $cate['id'];?>" class="mr5 J_ajax_del" data-pdata="{'sid': '<?php echo $cate['id'];?>'}">[删除]</a></td>
            </tr>
        </tbody>
        <?php if(isset($childrenlist[$cate['id']]) && $childrenlist[$cate['id']]):?>
        <tbody id="J_table_list_<?php echo $cate['id'];?>">
            <?php //var_dump($childrenlist);exit;?>
            <?php foreach($childrenlist[$cate['id']] as $ke=>$form):?>
            <tr id="J_forum_tr_<?php echo $form['id'];?>">
                <td></td>
                <td>
                <?php echo $lang[$form['type']];?><span class="plus_icon plus_end_icon"></span>
                <span class="mr10 J_forum_names" data-id="<?php echo $form['id'];?>"><?php echo $form['category_name'];?></span>
                <?php if($form['type'] == 'forum'):?>
                    <a style="display:none" data-id="<?php echo $form['id'];?>" data-type="forum_child" data-forumlevel="3" data-html="tr" href="#" class="link_add J_addChild">添加二级分类</a>
                <?php elseif($form['type'] == 'sub'):?>
                    <a style="display:none" data-id="<?php echo $form['id'];?>" data-type="forum_child" data-forumlevel="4" data-html="tr" href="#" class="link_add J_addChild">添加三级分类</a>
                <?php endif;?>
                </td>
                <td class="tar"><?php echo $form['id'];?></td>
                <td><?php echo $form['postcount'];?></td>
                <td>
                <a href="javascript:void(0)" target="_blank" class="mr5">[访问]</a>
                <a href="<?php echo HostName_Backed;?>/Category/DeleteCategory?sid=<?php echo $form['id'];?>" class="mr5 J_ajax_del" data-pdata="{'sid': '<?php echo $form['id'];?>'}">[删除]</a>
                </td>
            </tr>
            <?php endforeach;?>
...
<td>\
    ...
    <a style="display:none" data-nameid="NEW_ID_" class="link_add J_addChild" href="#" data-html="tbody" data-type="forum_child" data-id="temp_root_" data-forumlevel="2">添加子分类</a>\
</td>\
<td class="tar"></td>\
...
function forumChild(forum_level, a_id, name_id){
    ...
    <span class="plus_none_icon"></span>\
...
$(function(){
    ...
    $('#J_table_list').on('blur', '.J_forum_names_input', function() {
        ...
        //判断版块名是否修改过
        if($this.val() !== org_val) {
            $.post("<?php echo HostName_Backed;?>/service/editservicecategory", {sid: $this.data('id'), name: $this.val() }, function(data){
                if(data.state === 'success') {
                    alert('1111');
                    restore();
                }else if(data.state === 'fail'){
                    alert('222');
                    Wind.dialog.alert('已经有该服务名称,请更换其它名称！');
                    restore();
                    return false;
                }
            });
    ...
    */
    ...
    function search_suggestion(search_data) {
        ...
        if(search_text.length >= 2) {
            ...
            $.post('http://phpwind.com/admin.php?m=bbs&c=setforum&a=searchforum', {'keyword':search_data}, function(data) {

                if(data.state === 'success') {
                    var push_html = [];
                    $.each(data.data, function(i, o){
                        push_html.push('<li><a href="http://phpwind.com/admin.php?m=bbs&c=setforum&a=edit&fid='+ o.fid +'">'+ o.name +'</a></li>');
...
<?php //$this->renderpartial('/include/footer');?><!--加载尾部-->
